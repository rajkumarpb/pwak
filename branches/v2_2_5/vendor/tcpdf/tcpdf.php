...
if (!class_exists('TCPDF')) {
	 ...
	 */ 
	define('PDF_PRODUCER','TCPDF 4.4.006_PHP4 (http://www.tcpdf.org)');
	...
	*/
	class TCPDF {
		...
		*/
		 ...
		 */
		var $htmlLinkColorArray = array(0, 0, 255);
		...
		* @see endPage(), AddPage()
		*/
		function startPage($orientation='', $format='') {
		...
		*/
		function Annotation($x, $y, $w, $h, $text, $opt=array('Subtype'=>'Text'), $spaces=0) {
		...
		*/
		function MultiCell($w, $h, $txt, $border=0, $align='J', $fill=0, $ln=1, $x='', $y='', $reseth=true, $stretch=0, $ishtml=false, $autopadding=true, $maxh=0) {	
		...
		* @since 4.4.002 (2008-12-09)
		*/
		function getBorderMode($border, $position='start') {
		...
		* @since 1.5
		*/
		function Write($h, $txt, $link='', $fill=0, $align='', $ln=false, $stretch=0, $firstline=false, $firstblock=false, $maxh=0) {
	 ...
	 /**
		...
		* @since 1.1
		*/
		function Image($file, $x='', $y='', $w=0, $h=0, $type='', $link='', $align='', $resize=false, $dpi=300, $palign='', $ismask=false, $imgmask=false) {
			if ($x === '') {
				$x = $this->x;
			}
			if ($y === '') {
		...
		* @see Image()
		*/
		function ImagePngAlpha($file, $x='', $y='', $w=0, $h=0, $type='', $link='', $align='', $resize=false, $dpi=300, $palign='') {
			// get image size
		...
		*/
		 ...
		 * @access public
		 */
		function addHtmlLink($url, $name, $fill=0, $firstline=false, $color='', $style=-1) {
			if ($url{0} == '#') {
		...
		*/
		function getGroupPageNoFormatted() {
	...
	Putting 1 is equivalent to putting 0 and calling Ln() just after. Default value: 0.
		 ...
		 * @access protected
		 */
		function addHTMLVertSpace($n, $cell=false, $h='', $firstorlast=false, $tag='', $closing=false) {
			if ($firstorlast) {
				$this->Ln(0, $cell);
				$this->htmlvspace = 0;
				return;
		...
		* @since 4.2.000 (2008-10-29)
		*/
		function SetBooklet($booklet=true, $inner=-1, $outer=-1) {
		...
		* @since 4.2.000 (2008-10-29)
		*/
		function swapMargins($reverse=true) {
			if ($reverse) {
		...
		* @since 4.2.007 (2008-11-12)
		*/
		function setListIndentWidth($width) {
		...
		* @since 4.4.003 (2008-12-09)
		*/
		function setHtmlLinksStyle($color=array(0,0,255), $fontstyle='U') {
		...
		*/
        function getHTMLUnitToPoints($htmlval, $refsize=1, $defaultunit='px') {
		...
		* @since 4.4.004 (2008-12-10)
		*/
		function putHtmlListBullet($listdepth, $listtype='', $size=10) {
		    $fill = '';
		    $color = array(0, 0, 0);
		    $width = 0;
		    $textitem = '';
